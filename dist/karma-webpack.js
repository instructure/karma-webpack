...
const getOutputPath = outputPath => {
  for (let i = 0; i < outputPath.length; i++) {
    if (outputPath[i].indexOf('.js') !== -1 && outputPath[i].indexOf('.js.map') === -1) {
      return outputPath[i];
    }
  }
...
singleRun,
/* config.colors */
colors, customFileHandlers, emitter) {
  webpackOptions = cloneDeep(webpackOptions);
  webpackMiddlewareOptions = cloneDeep(webpackMiddlewareOptions || webpackServerOptions || {});
  const applyOptions = Array.isArray(webpackOptions) ? webpackOptions : [webpackOptions];
  const includeIndex = applyOptions.length > 1;
  applyOptions.forEach((webpackOptions, index) => {
    // The webpack tier owns the watch behavior so we want to force it in the config
    webpackOptions.watch = !singleRun; // Webpack 2.1.0-beta.7+ will throw in error if both entry and plugins are not specified in options
    ...
    const publicPath = indexPath !== '' ? `${indexPath}/` : ''; // Must have the common _karma_webpack_ prefix on path here to avoid
  ...
  webpackMiddlewareOptions.publicPath = '/_karma_webpack_/'; // Set webpack's color config to value specified in Karma's config for consistency
...
Plugin.prototype.make = function (compilation, callback) {
  this.entries.clear();
  async.each(this.files.slice(), (file, callback) => {
    let entry = file;
    ...
    const filename = normalize(path.relative(this.basePath, file).replace(/\\/g, '/'));
    const name = path.join(path.dirname(filename), path.basename(filename, path.extname(filename)));
  ...
  const doRead = function () {
    if (optionsCount > 1) {
      async.times(optionsCount, (idx, callback) => {
        middleware.fileSystem.readFile(path.join(os.tmpdir(), '_karma_webpack_', String(idx), this.outputs.get(file)), callback);
      }, (err, contents) => {
        if (err) {
          return callback(err);
        }
...
basePath, webpackPlugin) {
  return function (content, file, done) {
    ...
    const filename = path.relative(basePath, file.originalPath); // read blocks until bundle is done
    ...
    webpackPlugin.readFile(filename, (err, content) => {
      ...
      const outputPath = webpackPlugin.outputs.get(normalize(filename.replace(/\\/g, '/')));
